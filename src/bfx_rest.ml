...
let base_url = Uri.of_string "https://api.bitfinex.com"
...
module Sym = struct
  ...
  let get () =
    let uri = Uri.with_path base_url "/v1/symbols_details" in
...
let post_exn ?buf ~key ~secret ~endp ~body () =
  Logs_async.debug ~src (fun m -> m "-> %a" pp_json body) >>= fun () ->
  let uri = Uri.with_path base_url endp in
  ...
  let body_b64 = Base64.encode_exn body in
  let signature =
  ...
  Client.post ~headers ~body uri >>= fun (_resp, body) ->
  Body.to_string body >>= fun body ->
  Logs_async.debug ~src (fun m -> m "<- %s" body) >>| fun () ->
...
module Order = struct
  ...
  let submit_exn
    ...
    in
    post_exn ?buf ~key ~secret ~endp:"/v1/order/new" ~body

  let cancel_exn ?buf ~key ~secret oid =
    let body = `Assoc ["order_id", `Int oid] in
    post_exn ?buf ~key ~secret ~endp:"/v1/order/cancel" ~body
